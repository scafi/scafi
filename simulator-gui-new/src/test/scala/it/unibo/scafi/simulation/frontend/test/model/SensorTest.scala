...
import it.unibo.scafi.simulation.frontend.model.aggregate.AggregateEvent.NodeDeviceChanged
import it.unibo.scafi.simulation.frontend.model.sensor.SensorConcept.{sensorInput, sensorOutput}
import it.unibo.scafi.simulation.frontend.test.help.SensorWorldImpl
...
class SensorTest extends FunSpec with Matchers {
  ...
  checkThat("i can create a sensor network") {
    ...
    assert(world(0).get.getDevice(ledSensor).get.value[Any] != world(1).get.getDevice(ledSensor).get.value[Any])
  }
