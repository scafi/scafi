...
trait AggregateConcept {
  ...
  /**
   * this trait describe a mutable node, it must be protected because the node state can be changed only by world.
  ...
  protected trait AggregateMutableNode extends RootMutableNode {
     ...
     *   device name
     * @return
     *   None if the device isn't attach on the node Some of device otherwise
     */
   ...
   * @param events
   *   the events accepted by this observer
   */
