...
import it.unibo.scafi.simulation.SimulationObserver.SensorChangedEvent

class PlatformMonitor(
    override val space: SPACE[ID],
  ...
  private def updateNbrs(ids: Set[ID]): Unit = ids.foreach { id =>
    devicesLocation(id).foreach { ref =>
      ref ! I.MsgNeighborhoodLocations(
        id,
        neighbourhood(id)
          .filter(nbr => devicesLocation(nbr).isDefined)
          .map(nbr => nbr -> devicesLocation(nbr).get.path.toString)
          .toMap
      )
    }
  }
