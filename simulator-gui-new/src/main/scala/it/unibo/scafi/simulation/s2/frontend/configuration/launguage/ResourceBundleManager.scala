package it.unibo.scafi.simulation.s2.frontend.configuration.launguage

import java.util.Locale
import java.util.ResourceBundle

/**
 * a manger used to manage different language you change use resource manager in two way or used a string context <pre>
 * {@code implicit val file = KeyFile.Error val nameSolved = i"value" } </pre>
 *
 * or directly <pre> {@code implicit val file = KeyFile.Error val nameSolved = internation("value") { </pre>
 */
object ResourceBundleManager {
   ...
   *   the key name
   * @param file
   *   the file where manager find word
   * @return
   *   string founded
   */
