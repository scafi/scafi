...
import it.unibo.scafi.simulation.s2.frontend.model.aggregate.AggregateEvent.NodeDeviceAdded
import it.unibo.scafi.simulation.s2.frontend.model.aggregate.AggregateEvent.NodeDeviceRemoved
import it.unibo.scafi.simulation.s2.frontend.model.aggregate.AggregateEvent.NodesMoved
...
/**
 * describe a simulation presenter. allow to output graphics information about node changing (move, device update...)
 * you can choose to render or not neighbour and the output where to put world changes
 * @param world
 *   to show
 * @param neighbourRender
 *   a boolean value to define if show the neighbours or not
 */
class SimulationPresenter[W <: SensorPlatform](val world: W, val neighbourRender: Boolean)
    extends Presenter[W, SimulationView] {
  ...
  override def onTick(float: Float): Unit = {
    ...
    (devChanged.deviceChanged() ++ devAdded.deviceChanged()) map { x =>
      x._1 -> {
        x._2 foreach { name =>
