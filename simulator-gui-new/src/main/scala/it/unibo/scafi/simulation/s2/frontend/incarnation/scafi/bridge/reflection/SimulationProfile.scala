package it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.bridge.reflection
...
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.ScafiCommandBinding.AdHocToggleBinding
...
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.bridge.ExportEvaluation
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.bridge.MetaActionProducer
...
import it.unibo.scafi.simulation.s2.frontend.view.AbstractKeyboardManager.Code1
...
trait SimulationProfile {
  /**
   * @return
   *   the simulation command mapping
   */
  def commandMapping: CommandBinding

  /**
   * @return
   *   a sensor seed used to initialize sensors
   */
  def sensorSeed: Iterable[scafiWorld.DEVICE_PRODUCER]

  /**
   * @return
   *   a list of actuator uses into scafi world
   */
  def actions: List[MetaActionProducer[_]]

  /**
   * @return
   *   a list of export evaluation uses to valutate and produce change in gui world
   */
...
object SimulationProfile {
  ...
  /**
   * standard scafi profile : there is a set of on off input sensor and a set of any output sensor a possible scenario:
  ...
  /**
   * a scafi profile that describe a simulation with one on off sensor and any output sensor a possible scenario: you
   * want create a simulation with only an on off device with only one type of output
   */
  object onOffInputAnyOutput extends SimulationProfile {
    ...
    override val sensorSeed: Iterable[scafiWorld.DEVICE_PRODUCER] = ScafiDeviceProducers.ahHocDeviceConfiguration(
      List((sensor1, false, SensorConcept.sensorInput), (output1, "", SensorConcept.sensorOutput))
    )
  ...
  /**
   * a scafi profile that describe a simulation with moving nodes a possible scenario: you want create a simulation to
   * show a set of drone that are moving in the world with some logic
   */
