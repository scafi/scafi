...
import it.unibo.scafi.simulation.s2.frontend.view.WindowConfiguration.Size
...
trait WindowConfiguration {
  ...
  /**
   * @return
   *   None if the application hasn't a logo path Some(path) otherwise
   */
  def logoPath: Option[String]

  /**
   * @return
   *   None if the application hasn't an icon Some(path) otherwise
   */
...
object WindowConfiguration {
   ...
   *   the window created
   */
  def apply(width: Int, height: Int): WindowConfiguration =
   ...
   * @return
   *   a rectangle with the width and height passed
   */
  implicit def toWindowRect(windowConfiguration: WindowConfiguration): Rectangle = windowConfiguration.size match {
    case FullScreen =>
      Rectangle(Toolkit.getDefaultToolkit.getScreenSize.width, Toolkit.getDefaultToolkit.getScreenSize.height)
