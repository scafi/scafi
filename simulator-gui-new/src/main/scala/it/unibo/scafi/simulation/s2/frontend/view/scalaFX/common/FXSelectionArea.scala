...
private[scalaFX] trait FXSelectionArea extends AbstractSelectionArea {
  ...
  this.handleEvent(MouseEvent.Any) { me: MouseEvent =>
      ...
      me.eventType match {
        ...
        case MouseEvent.MouseReleased =>
          if (this.circle.isDefined && this.moved.isEmpty) {
            this.moved = this.nodes.filter(x => this.circle.get.contains(nodeToPosition(x._2))).map { x =>
              x._1 -> RichNode(x._2).cloneNode()
            }
            ...
            this.moved.nonEmpty && (this.circle.get.translateX.value != 0 || this.circle.get.translateY.value != 0)
          ) {
            val toMove = moved.map(x => x._1 -> view.scalaFX.nodeToPosition(x._2)) map { x =>
              x._1 -> pointToWorldPosition(x._2)
            }
