...
/**
 * define a trait of aggregate world, you can make some actuation like move node and attach/ detach device on node
 */
trait AggregateWorld extends ObservableWorld with AggregateConcept {
   ...
   *   if the node isn't in the world
   * @return
   *   true if the movement is allowed false otherwise
   */
