...
trait AsyncLogicController[W <: AggregateWorld] extends LogicController[W] {
  /**
   * @return
   *   the name of current async logic
   */
  def asyncLogicName: String
  /**
   * @return
   *   min delta value admissible
   */
  protected def minDelta: Int = 0
  /**
   * @return
   *   max delta value admissible
   */
   ...
   * @return
   *   true if the logic is stopped false otherwise
   */
