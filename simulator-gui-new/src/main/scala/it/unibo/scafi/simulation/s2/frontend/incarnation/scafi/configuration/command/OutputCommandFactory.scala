 ...
 * @param outputs
 *   the outpolicy supported
 * @param scafiConfiguration
 *   implicit scafi configuration
 */
class OutputCommandFactory(outputs: OutputPolicy*)(implicit val scafiConfiguration: ScafiConfigurationBuilder)
  ...
  override protected def createPolicy(args: CommandArg): (Result, Option[Command]) = args.get(Name) match {
    case Some(value: String) =>
      if (outputMap.get(value).isDefined) {
