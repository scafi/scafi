...
object SensorConcept {
  ...
  trait SensorType {
     ...
     * @param arg
     *   a new value of sensor
     * @return
     *   true if the value is legit false otherwise
     */
  ...
  /**
   * describe the stream of sensor, can be inputStream or OutputStream sensor stream has this task: node can use sensor:
   * as acquisition (the node check the value and produce a result) as writable output (after computation node change
   * the state of a sensor of this type)
   */
