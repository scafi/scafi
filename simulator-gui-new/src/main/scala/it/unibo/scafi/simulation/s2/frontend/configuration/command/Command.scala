...
/**
 * command pattern, a logic encapsulated inside a instance of command a command can be undone
 */
trait Command {
  /**
   * the execution command logic
   * @return
   *   Success if there isn't error Fail otherwise
   */
  def make: () => Result
