 ...
 *   the scafi configuration
 */
class RadiusSimulationCommandFactory(implicit val scafiConfiguration: ScafiConfigurationBuilder)
  ...
  override def commandArgsDescription: Seq[CommandFactory.CommandArgDescription] =
    List(
      CommandArgDescription(DemoValue, LimitedValueType(scafiDemo: _*), description = international(name, DemoValue)),
  ...
  override protected def createPolicy(args: CommandArg): (Result, Option[Command]) = {
    ...
    args.get(DemoValue) match {
      case Some(stringFrom: String) if ListDemo.nameToDemoClass.get(stringFrom).isDefined =>
        demoClass = ListDemo.nameToDemoClass.get(stringFrom)
    ...
    if (demoClass.isDefined && radius.isDefined) {
      easyResultCreation { () =>
        ...
        scafiConfiguration.scafiSimulationInformation = Some(
          SimulationInfo(
            program = demoClass.get,
