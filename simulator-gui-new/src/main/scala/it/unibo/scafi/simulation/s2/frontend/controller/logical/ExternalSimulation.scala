 ...
 */
abstract class ExternalSimulation[W <: AggregateWorld](override val asyncLogicName: String)
    extends AsyncLogicController[W] {
  ...
  /**
   * @return
   *   the current simulation prototype
   */
   ...
   *   if there isn't simulation stopped
   */
  def continue(): Unit = {
  ...
  trait ExternalSimulationContract {
     ...
     *
     * @return
     *   None if the simulation isn't initialize, Some otherwise
     */
     ...
     *   if the simulation is already initialized
     * @param prototype
     *   the prototype used to create simulation
     */
     ...
     *
     * @throws IllegalStateException
     *   if the simulation is never initialize
     * @param prototype
     *   the prototype used to restart simulation
     */
