...
import it.unibo.scafi.simulation.s2.frontend.configuration.environment.ProgramEnvironment.PerformancePolicy
...
import it.unibo.scafi.simulation.s2.frontend.configuration.Program
import it.unibo.scafi.simulation.s2.frontend.configuration.ProgramBuilder
...
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.bridge.ScafiSimulationInitializer
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.bridge.SimulationInfo
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.world.ScafiLikeWorld
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.world.ScafiWorldInitializer
 ...
 */
private class ScafiProgramBuilder(override val configuration: ScafiConfiguration)
    extends ProgramBuilder[ScafiConfiguration] {
  override def create: Program[_, _] = {
    ...
    val programEnv =
      new ScafiProgramEnvironment(presenter, bridged, configuration.performance, configuration.logConfiguration)
...
object ScafiProgramBuilder {
  /**
   * allow to create a program passing some parameter you can launch a scafi aggregate application like this: <pre>
   * {@code ScafiProgramBuilder ( Random(500,500,500), SimulationInfo(program = classOf[Simple]),
   * RadiusSimulation(radius = 40) ).launch() } </pre>
   * @param scafiWorldInfo
   *   the world seed in scafi context
   * @param worldInitializer
   *   a world initializer to initialize scafi world
   * @param commandMapping
   *   a command mapping used to map some keyboard (and selection) input to an action
   * @param scafiSimulationInfo
   *   a seed used to set some parameter in scafi bridge simulation
   * @param simulationInitializer
   *   a simulation initializer used to initialize scafi bridge simulation
   * @param outputPolicy
   *   strategy describe how to output some information
   * @param neighbourRender
   *   allow to render or not neighbour
   * @param performance
   *   the perfomance of program context
   * @return
   *   the program create by information passed
   */
  def apply(
      ...
      performance: PerformancePolicy = StandardPolicy
  ): Program[_, _] = {
