...
import it.unibo.scafi.simulation.s2.frontend.incarnation.scafi.configuration.ScafiWindowInfo
...
import it.unibo.scafi.simulation.s2.frontend.view.scalaFX.drawer.ImageFXOutput
...
object GraphicsLauncher extends App {
  ...
  val factories: List[CommandFactory] = List(radiusFactory, gridFactory, randomFactory, outputFactory,
    performanceFactory, windowConfigurationFactory, logConfiguration, renderConfiguration)

  val parser = new UnixLikeParser(new LaunchCommandFactory :: factories: _*)
  val machine = new ConfigurationMachine(parser)
  val map: Map[String, List[CommandFactory]] = Map(
    international("world-initializer")(KeyFile.Configuration) -> List(randomFactory, gridFactory)
  )
