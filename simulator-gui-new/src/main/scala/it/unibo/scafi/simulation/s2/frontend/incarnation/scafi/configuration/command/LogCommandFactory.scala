 ...
 *   the log configuration accepted
 * @param scafiConfiguration
 *   the scafi configuration builder
 */
class LogCommandFactory(logConfiguration: LogConfiguration*)(implicit val scafiConfiguration: ScafiConfigurationBuilder)
  ...
  override def commandArgsDescription: Seq[CommandFactory.CommandArgDescription] =
    List(
      CommandArgDescription(
        ConfigurationName,
        typeAccepted,
        description = international(name, ConfigurationName),
        defaultValue = Some(scafiConfiguration.logConfiguration.toString)
      )
    )

  override protected def createPolicy(args: CommandArg): (Result, Option[Command]) = args.get(ConfigurationName) match {
    case Some(name: String) if logMapped.contains(name) =>
      easyResultCreation(() => scafiConfiguration.logConfiguration = logMapped(name))
