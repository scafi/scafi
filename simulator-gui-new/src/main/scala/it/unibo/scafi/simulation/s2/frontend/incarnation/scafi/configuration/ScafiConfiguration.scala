...
import it.unibo.scafi.simulation.s2.frontend.configuration.Configuration
import it.unibo.scafi.simulation.s2.frontend.configuration.Configuration.ConfigurationBuilder
 ...
 * @param scafiSimulationInformation
...
object ScafiConfiguration {
  ...
  /**
   * scafi configuration builder, has the same configuration value, are optional because some of these can be empty or
   * not, to argument description {@see ScafiConfiguration}
   * @param scafiWorldInfo
   *   (standard => ScafiSeed.standard)
   * @param worldInitializer
   *   (no standard value)
   * @param commandMapping
   ...
   *   (no standard value)
   * @param outputPolicy
   *   (standard => StandardFXOutputPolicy)
   * @param neighbourRender
   *   (standard => true)
   * @param performance
   *   (standard => NearRealTimePolicy)
   */
  class ScafiConfigurationBuilder(
      var scafiWorldInfo: ScafiWorldInformation = ScafiWorldInformation.standard,
      ...
      var performance: PerformancePolicy = NearRealTimePolicy
  ) extends ConfigurationBuilder[ScafiConfiguration] {
    ...
    def create(): Option[ScafiConfiguration] = {
      if (worldInitializer.isEmpty || simulationInitializer.isEmpty) {
        ...
        Some(
          ScafiConfiguration(
            scafiWorldInfo,
            worldInitializer.get,
            commandMapping,
            scafiSimulationInformation.get,
            simulationInitializer.get,
            outputPolicy,
            neighbourRender,
            logConfiguration,
            performance
