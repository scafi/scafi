...
import it.unibo.scafi.simulation.s2.frontend.configuration.parser.AnyParser
...
/**
 * an input controller that execute command it has a oommand queue that is process when scheduler tick this concept.
 * support undo action to remove last command chages
 */
object InputCommandController extends InputController {
   ...
   * @param command
   *   the command to add into queue list
   */
  ...
  object UndoCommandFactory extends CommandFactory {
    ...
    override protected def createPolicy(args: CommandArg): (Result, Option[Command]) =
      CommandFactory.creationSuccessful(UndoCommand)
  }
