...
import it.unibo.scafi.space.graphics2D.BasicShape2D.{Circle => InternalCircle}
import it.unibo.scafi.space.graphics2D.BasicShape2D.{Polygon => InternalPolygon}
import it.unibo.scafi.space.graphics2D.BasicShape2D.{Rectangle => InternalRectangle}
...
package object scalaFX {
  ...
  implicit class RichNode(node: Node) {
    ...
    def cloneNode(): Node = {
      node.delegate match {
        case rect: Rectangle =>
          val rectCloned = new Rectangle(
            rect.x.value + rect.translateX.value,
          ...
          rectCloned
        case circle: Circle =>
          val circleCloned = new Circle(
            circle.centerX.value + circle.translateX.value,
