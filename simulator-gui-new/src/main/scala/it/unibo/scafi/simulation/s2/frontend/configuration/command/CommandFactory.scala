...
/**
 * a factory used to create a set of command with a factory you can create command like these:
 *
 * <pre> {@code val creationResult = factory.create(Map("arg1" -> value1, "arg2" -> value")) } </pre>
 *
 * factories isn't used directly (in general) but it is used for example in
 * @see
 *   it.unibo.scafi.simulation.gui.configuration.parser.UnixLikeParser
 */
trait CommandFactory {
   ...
   * @return
   *   a sequence of command argument description
   */
  def commandArgsDescription: Seq[CommandArgDescription]

  /**
   * this method is used to create a command, the argument is a map of String -> Any, the factory check if the argument
   * if accepted and create the command associated with the factory
   * @param args
   *   the command argument
   * @return
   *   (Success,Some(Command)) if the arguments are accepted (Fail,None) otherwise
   */
...
object CommandFactory {
   ...
   *   the argument type
   * @param optional
   *   a boolean value that tells if the argument is optional or not
   * @param description
   *   a description of argument
   * @param defaultValue
   *   the default value associated with the command argument
   */
  case class CommandArgDescription(
      name: String,
      valueType: ValueType,
   ...
   * @param expected
   *   the parameter names expected into the command arg
   * @return
   *   the string created
   */
   ...
   * @param commandLogic
   *   the internal logic of command
   * @return
   *   the creation result created
   */
  def easyResultCreation(commandLogic: () => Unit): CreationResult = (
    Success,
