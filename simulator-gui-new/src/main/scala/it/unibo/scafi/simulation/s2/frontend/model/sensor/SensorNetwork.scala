...
/**
 * describe a network of sensor attach on a node, you can produce actuation on the world by changed sensor value
 */
trait SensorNetwork extends SensorConcept {
   ...
   *   new value
   * @return
   *   true if the device is attached on node false otherwise
   * @throws IllegalArgumentException
   *   if the type sensor is not the same of parameter
   */
