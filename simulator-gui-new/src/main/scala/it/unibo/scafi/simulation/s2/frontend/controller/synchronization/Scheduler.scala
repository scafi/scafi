...
import it.unibo.utils.observer.PrioritySource
import it.unibo.utils.observer.Source

/**
 * Scheduler has internal frequency used to notify his observer. the observers are Controller, each controller must be
 * change the world only when scheduler notify it. the main reason of this structure is to separate the model update
 * with external update. i want to create a structure game loop like.
 */
