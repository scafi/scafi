...
import it.unibo.scafi.incarnations.BasicSimulationIncarnation.{AggregateProgram, BlockG, BlockS, BlockT, Builtins, FieldUtils, GenericUtils, LSNS_RANDOM, NBR_RANGE_NAME, StandardSensors}
...
object DemoPrograms extends App {
  StringLauncher{"grid-world " +
   "200 10 5;radius-simulation DemoMeanCounter 200"}
}
@Demo
...
class Timer extends AggregateProgram with BlockT {
  override def main() = Duration(
    branch(!sense[Boolean](SensorName.sensor1)){ timer(Duration(30, TimeUnit.SECONDS)) } { 0 },
