...
trait Stdlib_BlockF {
  ...
  trait BlockF {
    ...
    def flock(lastVec: (Double, Double), flockingSensor: Seq[Boolean], obstacleSensor: Seq[Boolean], separationDistance: Double,
      ...
      val n = reifyField(nbrVector())
      val activeNodes = foldhood[Seq[Point3D]](Seq[(Point3D)]())(_ ++ _){
        mux(nbr(getBooleanResult(flockingSensor)))(Seq(n.apply(nbr(mid().asInstanceOf[Int]))))(Seq())
      }
    ...
    private def obstacle(obstacleSensor: Seq[Boolean], n: Map[Int, Point3D]): (Double, Double) = {
      val obstaclesVector = foldhood[Seq[Point3D]](Seq[(Point3D)]())(_ ++ _){
        mux(nbr(getBooleanResult(obstacleSensor)))(Seq(n.apply(nbr(mid().asInstanceOf[Int]))))(Seq())
    ...
    private def alignment(lastVec: (Double, Double), flockingSensor: Seq[Boolean]): (Double, Double) = {
      val alignmentVector: (Double, Double) = foldhood((0.0, 0.0))((d1, d2) => (d1._1 + d2._1, d1._2 + d2._2))(nbr(mux(getBooleanResult(flockingSensor)) {
        lastVec
      } {
        (.0, .0)
      }))
    ...
    private def separation(separationDistance: Double, neighbors: Map[Int, Point3D]): (Double,Double) = {
    ...
    def getBooleanResult(sensor: Seq[Boolean]): Boolean = sensor.fold(false)((sensor1, sensor2) => sensor1 || sensor2)

    def reifyField[T](expr: => T): Map[Int, T] = foldhood[Seq[(Int, T)]](Seq[(Int, T)]())(_ ++ _) {
        Seq(nbr{mid().asInstanceOf[Int]} -> expr)
      }.toMap
