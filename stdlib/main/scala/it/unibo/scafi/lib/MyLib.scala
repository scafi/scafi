...
trait MyLib {
  ...
  trait MyLib {
    ...
    def findParent[V](potential: V)(implicit ev: Bounded[V]): ID = {
      mux(ev.compare(minHood {
        nbr(potential)
      }, potential) < 0) {
        minHood {
          nbr {
            Tuple2[V, ID](potential, mid())
          }
        }._2
      } {
        Int.MaxValue
    ...
    def field[A](expr: => A): List[A] = foldhood(List[A]())(_ ++ _) {
      List[A](expr)
    }

    def devField[A](expr: => A): Map[ID, A] = foldhood(Map[ID, A]())(_ ++ _) {
      nbr {
        Map[ID, A](mid() -> expr)
      }
    }
