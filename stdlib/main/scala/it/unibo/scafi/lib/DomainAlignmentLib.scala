...
trait DomainAlignmentLib {
  ...
  trait DomainAlignmentLib {
    ...
    def exec(procs: List[VersionedFunction], maxp: Int, curp: Int, nnum: Int, init: () => Double): (Double, Int) = aggregate {
      ...
      val d_nxt = x._1
      val ncurp: Int = x._2
      mux(ncurp < 0 && nnum == foldhood(0)(_ + _) {
        1
      }) {
        (d_cur, procs.head.ver)
      } {
        x
      }
    }
    ...
    def safeup(metric: () => VersionedFunction, init: () => Double) = aggregate {
      (rep((List[VersionedFunction](), -1, -1, nbr {
        0.0
      })) {
        case ((procs: List[VersionedFunction], maxp: Int, curp: Int, field: Double)) => aggregate {
          val y: (Int, List[VersionedFunction]) = foldhood((maxp, procs))((a, b) => if (a._1 >= b._1) a else b) {
            nbr {
              (maxp, procs)
            }
          }
          val z: (Int, List[VersionedFunction]) = mux(metric().ver > maxp) {
            (metric().ver, metric() :: procs)
          } {
            y
          }
          val nmaxp = z._1
          val nprocs = z._2
          val x = exec(nprocs, nmaxp, curp, foldhood(0)(_ + _) {
            1
          }, init)
    ...
    def crfgradient(source: Boolean, metric: () => Double) = aggregate {
      (rep(RaisingDist(inf, false)) {
        (d) =>
          aggregate {
            val x = foldhood(RaisingDist(inf, false))(_.min(_)) {
              nbr {
                d
              } + nbrRange()
            }
    ...
    def gradient(source: Boolean, metric: () => Double) = aggregate {
      rep(inf) {
        (dist) => mux(source) {
          ...
          foldhood(inf)(Math.min(_, _))(metric() + nbr {
            dist
          })
        }
      }
    ...
    def injmetric(): VersionedFunction = aggregate {
      if (mid() == 1 && time < 50) {
        /*print('a');*/ VersionedFunction(2, nbrRange)
      }
      else if (mid() == 2 && time > 50 && time < 100) {
        /*print('z');*/ VersionedFunction(3, smartmetric)
      }
    ...
    def testGsafe() = {
      gradient(mid() == 0, () => aggregate {
        safeup(() => injmetric, () => badmetric)
      })
    }

    def testCRFup() = {
      crfgradient(mid() == 0, () => aggregate {
        up(() => injmetric)()
      })
    }
