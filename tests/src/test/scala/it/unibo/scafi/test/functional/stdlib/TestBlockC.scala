...
class TestBlockC extends FunSpec with BeforeAndAfterEach {
  ...
  private[this] trait TestProgramStandard extends AggregateProgram with ScafiStandardLanguage with ScafiStandardLanguageLibraries
    with StandardSensors with BlockC {
  ...
  //TODO now it is copy-paste, can do better?
  describe(s"BlockC for ScafiFC") {
    describe("should support findParent") {
      it("should work with a constant value") {
        exec(new TestProgramFC {
          override def main(): ID = findParent(1)
        }, ntimes = defaultNtimes)(net)
