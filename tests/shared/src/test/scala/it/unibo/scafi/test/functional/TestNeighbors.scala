...
class TestNeighbors extends AnyFlatSpec with Matchers {
  ...
  for(s <- seeds) {
    ...
    behavior of s"Neighbours fields for $seeds"
  ...
  def behaviours(seeds: Seeds): Unit = {
    ...
    it should "be possible to count neighbors excluding self" in new SimulationContextFixture(seeds) {
      import node._
    ...
    it should "be possible for nodes to gather the IDs of their neighbors" in new SimulationContextFixture(seeds) {
      import node._
    ...
    it should "be possible for nodes to gather the IDs of their neighbors excluding themselves" in
    ...
    it should "be possible to calculate the min distance from neighbors, in a grid" in
        new SimulationContextFixture(seeds) {
      import node._
    ...
    it should "be possible to calculate the min distance from neighbors, in a ad-hoc net" in
        new SimulationContextFixture(seeds) {
      import node._
